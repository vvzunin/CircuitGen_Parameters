...
static std::vector<std::string> d_incorrectWords = {"ERROR"};
    ...
    for (auto currentCommand : i_info) {
      ...
      for (auto word : d_incorrectWords) {
        ...
        if (secondPos == std::string::npos && errorPos == std::string::npos) {
          std::string errText =
              "Something went wrong during files parsing in YosysUtils\n";
        ...
        if (errorPos != std::string::npos &&
            (errorPos < secondPos || secondPos == std::string::npos)) {
          int endPos = secondPos - errorPos - 1;
          if (secondPos == std::string::npos)
            endPos = result.size() - errorPos - 1;

          std::string errText = "Incorrect " + currentCommand.info + ": " +
                                result.substr(errorPos, endPos) + '\n';
...
std::vector<StandartCommandInfo> YosysUtils::parseCommand(
...
CommandWorkResult YosysUtils::optVerilog(std::string i_inputFileName,
                                         std::string i_outputFileName,
                                         std::string i_directory) {
  return runCommand(YosysCommands::optVerilogCommand, standartExecutor,
                    i_directory, i_inputFileName, i_directory,
                    i_outputFileName);
}

CommandWorkResult YosysUtils::writeFirrtl(std::string i_inputFileName,
                                          std::string i_outputFileName,
                                          std::string i_directory) {
  return runCommand(YosysCommands::writeFirrtlCommand, standartExecutor,
                    i_directory, i_inputFileName, i_outputFileName);
}