...
static std::vector<std::string> d_allowedOutput = {"read", "print", "map"};
// which words means errors
static std::vector<std::string> d_incorrectWords = {"Error", "failed",
                                                    "Cannot"};
...
CommandWorkResult AbcUtils::standartExecutor(
  ...
  if (i_command.find("2>&1") == std::string::npos) i_command += " 2>&1";
  ...
  if (firstPos != std::string::npos) {
    for (auto currentCommand : i_info) {
      ...
      if (secondPos == std::string::npos && currentCommand.info != "quit" &&
          delta + firstPos + 1 < result.size()) {
        std::string errText =
            "Something went wrong during files parsing in AbcUtils\n";
      ...
      for (int i = 0; i < d_allowedOutput.size() && !allowedToPrint; ++i)
        allowedToPrint =
            currentCommand.info.find(d_allowedOutput[i]) != std::string::npos;

      // If something went wrong during read
      // and abc wrote something
      if (!allowedToPrint && secondPos - firstPos > delta) {
        std::string errText =
            "Incorrect " + currentCommand.info + ": " +
            result.substr(firstPos + delta, secondPos - firstPos - delta) +
            '\n';
      ...
      // If we can print sth, we check, is there something illegal in this
      // output
        ...
        for (int i = 0; i < d_incorrectWords.size() && correct; ++i) {
          if (output.find(d_incorrectWords[i]) != std::string::npos) {
            std::string errText =
                "Incorrect " + currentCommand.info + ": " + output + '\n';
    ...
    std::string errText =
        "Something went wrong at start during files parsing in AbcUtils: \n";
    errText += result;
...
std::vector<StandartCommandInfo> AbcUtils::parseCommand(std::string i_command) {
  ...
  while (start != std::string::npos) {
    ...
    int commandNameEnd = std::min(i_command.find(' ', commandNameStart),
                                  i_command.find('"', commandNameStart));

    // sumLen: end is bigger on 1 than real i_command ending, start is on 1
    // smaller, but we have \n in the end, so we do not add 1 info: we use
    // substr, "commandNameEnd - commandNameStart + 1" will show i_command name
    // with a space
    StandartCommandInfo commandInfo = {
        .sumLen = end - start,
        .info = i_command.substr(commandNameStart,
                                 commandNameEnd - commandNameStart)};
...
CommandWorkResult AbcUtils::runExecutorForStats(
    std::string i_command, std::vector<StandartCommandInfo> i_info) {
  CommandWorkResult final_res = standartExecutor(i_command, i_info);
  ...
  if (final_res.correct && final_res.commandsOutput.count("print_stats")) {
    ...
    startPos = stats.find_first_not_of(' ', startPos + 1);
    {
      ...
      final_res.commandsOutput["inputs"] =
          stats.substr(startPos, endPos - startPos);

      startPos = stats.find_first_not_of(' ', endPos + 1);
      endPos = stats.find(' ', startPos + 1);

      final_res.commandsOutput["outputs"] =
          stats.substr(startPos, endPos - startPos);
    }
    ...
    while (startPos != std::string::npos) {
      ...
      final_res.commandsOutput[stats.substr(startPos, wordEnd - startPos)] =
          stats.substr(digitStart, digitEnd - digitStart);

      startPos = stats.find_first_not_of(' ', digitEnd + 1);
    }

    final_res.commandsOutput["fileRead"] = fileRead;
  } else if (final_res.correct &&
             !final_res.commandsOutput.count("print_stats")) {
    final_res.commandsOutput.clear();

    final_res.commandsOutput["error"] =
        "Incorrect output, no stats had been printed\n";
    final_res.commandsOutput["fileRead"] = fileRead;
...
CommandWorkResult AbcUtils::resyn2(std::string i_inputFileName,
                                   std::string i_libName,
                                   std::string i_fileDirectory,
                                   std::string i_libDirectory) {
  std::string real_name = i_inputFileName;
...
CommandWorkResult AbcUtils::optimizeWithLib(std::string i_inputFileName,
                                            std::string i_libName,
                                            std::string i_fileDirectory,
                                            std::string i_libDirectory) {
  std::string real_name = i_inputFileName;
